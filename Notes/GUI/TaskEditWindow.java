...
public class TaskEditWindow extends JFrame implements ActionListener{
    Color bgColor = new Color(0x210F60);
    Border border = BorderFactory.createLineBorder(bgColor, 5);
    JPanel notePanel = new JPanel();
    NoteButton saveButton = new NoteButton("Save Task");
    ...
    public TaskEditWindow(LinkedList<Note> notes, int u){
        this.notes = notes;
        this.u = u;
        noteEdit.setText(notes.get(u).getNote());
        nameField.setText(notes.get(u).getName());
        numberField.setText(((Task)(notes.get(u))).getDate());
        initial();
        this.add(notePanel);
        this.add(saveButton);
    ...
    private void save(){
        ...
        ((Task)(notes.get(u))).setDate(numberField.getText());
